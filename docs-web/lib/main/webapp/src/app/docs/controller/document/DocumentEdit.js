...
angular.module('docs').controller('DocumentEdit', function ($rootScope, $scope, $q, $http, $state, $stateParams, Restangular, $translate) {
  ...
  $scope.edit = function () {
    ...
    document.relations = _.pluck(_.where(document.relations, {
      source: true
    }), 'id');
    ...
    var attachOrphanFiles = function (data) {
      ...
      _.each($scope.orphanFiles, function (fileId) {
        promises.push(Restangular.one('file/' + fileId).post('attach', {
          id: data.id
        }));
      });
      $scope.orphanFiles = [];
        ...
        var sendFile = function (key) {
          ...
          deferred.promise.then(function (data) {
            ...
            $scope.alerts.unshift({
              type: 'danger',
              msg: $translate.instant('document.edit.document_' + ($scope.isEdit() ? 'edited' : 'added') + '_with_errors') + (data.responseJSON && data.responseJSON.type === 'QuotaReached' ? ' - ' + $translate.instant('document.edit.quota_reached') : '')
            });
