...
angular.module('docs').controller('SettingsUserEdit', function ($scope, $dialog, $state, $stateParams, Restangular, $translate) {
  ...
  $scope.edit = function () {
    ...
    if ($scope.isEdit()) {
      promise = Restangular.one('user', $stateParams.username).post('', user);
    } else {
      ...
      if (result === 'ok') {
        Restangular.one('user', $stateParams.username).remove().then(function () {
          ...
          if (e.data.type === 'UserUsedInRouteModel') {
            ...
            var msg = $translate.instant('settings.user.edit.user_used_message', {
              name: e.data.message
            });
            var btns = [{
              result: 'ok',
  ...
  $scope.passwordReset = function () {
    Restangular.one('user').post('password_lost', {
      ...
      var msg = $translate.instant('settings.user.edit.password_lost_sent_message', {
        username: $stateParams.username
      });
      var btns = [{
        result: 'ok',
