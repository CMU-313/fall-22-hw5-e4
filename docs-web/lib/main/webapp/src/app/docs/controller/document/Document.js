...
angular.module('docs').controller('Document', function ($scope, $rootScope, $timeout, $state, Restangular, $q, $filter, $uibModal) {
  ...
  $scope.$watch('search', function () {
    ...
    if ($state.current.name === 'document.default' || $state.current.name === 'document.default.search') {
      $state.go($scope.search === '' ? 'document.default' : 'document.default.search', {
        search: $scope.search
      }, {
        location: 'replace',
        notify: false
  ...
  // Tag navigation
  $scope.tags = [];
  $scope.navigatedFilter = {
    parent: ''
  };
  $scope.navigatedTag = undefined;
  $scope.navigationEnabled = _.isUndefined(localStorage.navigationEnabled) ? true : localStorage.navigationEnabled === 'true';
  Restangular.one('tag/list').get().then(function (data) {
    $scope.tags = data.tags;
    _.each($scope.tags, function (tag) {
      tag.children = _.where($scope.tags, {
        parent: tag.id
      });
    });
    $scope.extractNavigatedTag();
  ...
  $scope.navigateUp = function () {
    ...
    $scope.navigateToTag(_.findWhere($scope.tags, {
      id: $scope.navigatedTag.parent
    }));
  };
  ...
  $scope.getCurrentNavigation = function () {
    ...
    while (current.parent) {
      current = _.findWhere($scope.tags, {
        id: current.parent
      });
      if (!current) {
        break;
  ...
  $scope.extractNavigatedTag = function () {
    ...
    if (tagFound) {
      ...
      $scope.navigatedTag = _.findWhere($scope.tags, {
        name: tagFound
      });
    } else {
      $scope.navigatedTag = undefined;
